/**
 * Small request-body field validators shared by the CRUD routes. The package-import path has its
 * own strict parsers; these bring the direct create/update endpoints (which previously assigned
 * several fields with no type checks) up to the same bar.
 */

/** Thrown by a validator when a field has the wrong shape; routes turn it into a 400. */
